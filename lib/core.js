...
function buildBannerHtml(options) {
    ...
    // Diagonal banners: top-left to bottom-right (\) or bottom-left to top-right (/)
    if (position === 'diagonal' || position === 'diagonal-tlbr' || position === 'diagonal-bltr') {
        const opacity = options.opacity !== undefined ? options.opacity : 0.5;
        // diagonal or diagonal-tlbr: top-left to bottom-right (\) uses 45deg
        // diagonal-bltr: bottom-left to top-right (/) uses -45deg
        const rotation = position === 'diagonal-bltr' ? '-45deg' : '45deg';
        css = `
        #env-banner-bar {
            position: fixed;
            top: 50%;
