 ...
 * @param {('top'|'bottom'|'top-left'|'top-right'|'bottom-left'|'bottom-right'|'diagonal'|'diagonal-tlbr'|'diagonal-bltr')} [options.position='bottom'] - Position of the banner.
 * @param {boolean} [options.showHost=true] - Whether to display the hostname/port in the banner.
 * @param {number} [options.opacity] - Banner opacity from 0.0 (transparent) to 1.0 (opaque). Defaults to 1.0 for most positions, 0.5 for diagonal.
